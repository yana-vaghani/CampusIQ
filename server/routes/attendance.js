...
// GET /api/attendance/subject/:subjectId — MUST be before /:studentId
router.get('/subject/:subjectId', auth, async (req, res) => {
  try {
    const { date } = req.query;
    let query = `
      SELECT a.*, s.roll_no, u.name as student_name, s.id as student_id
      FROM attendance a
...
// POST /api/attendance — teacher marks attendance (bulk upsert)
  ...
  try {
    ...
    for (const r of records) {
      await pool.query(
        ...
        [r.studentId, r.subjectId, r.date, r.status || 'absent']
      );
      count++;
    }
